...
module V1
  RSpec.describe ClubsController, type: :controller do
    ...
    describe 'GET #show' do
      let(:fb_attr) { 'official_facebook_page' }
      let(:web_attr) { 'official_website' }
      let(:tw_attr) { 'official_twitter_page' }
      ...
      it 'returns a club passed by param' do
        ...
        expect(response.body[web_attr]).to eq(club[web_attr])
        expect(response.body[fb_attr]).to eq(club[fb_attr])
        expect(response.body[tw_attr]).to eq(club[tw_attr])
      end
    end
    ...
    describe 'PUT #update' do
      context 'with valid params' do
        let(:path) { 'spec/support/images/clubs/shield-new.png' }
        let(:shield) { fixture_file_upload(File.join(Rails.root, path)) }
        let(:new_attributes) { attributes_for :club, shield: shield }
