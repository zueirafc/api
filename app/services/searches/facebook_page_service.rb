  ...
  class FacebookPageService
    class << self
      def find_posts_for(event)
        ...
        Instagram.client.tag_recent_media(event.hash_tag, params)
          .each do |media|
          begin
            ActiveRecord::Base.transaction do
              event.items << make_a_item_by(media)
            end
          rescue => e
      ...
      def make_an_author_by(user)
        author = Author
                 .find_or_create_by!(id: user.id.to_i,
        ...
        author.remote_avatar_url =
          user.profile_picture.to_s unless author.avatar?
